...
export default {
  ...
  data() {
    return {
      inductionId: '', // ID of the induction item to edit
      inductionName: '',// Separate property for Induction Name 
      description: '', // Separate property for Type of Induction
      errorMessage: '', // Error Message
    };
  },
  methods: {
    fetchData() {
      // Fetch data of the selected induction item from API and populate the form fields
      axios.get(`http://127.0.0.1:8000/inductionItems/${this.inductionId}`)
        .then(response => {
          const { name, description } = response.data;
          this.inductionName = name;
    ...
    updateItem() {
      // Update the induction item with the new data
      axios.put(`http://127.0.0.1:8000/inductionItems/${this.inductionId}`, {
  ...
  mounted() {
    // Get induction item ID from route params or props
    this.inductionId = this.$route.params.inductionId; // using Vue Router
  }
};
</script>
